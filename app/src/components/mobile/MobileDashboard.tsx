...
export default function MobileDashboard({ onLogout }: { onLogout?: () => void }) {
    ...
    (async () => {
      try {
        unlisten = await onOpenUrl((urls) => {
          if (urls.length > 0) {
            const url = urls[0];
            toast.success(`已收到 Telegram 链接：${url}`, { duration: 5000 });
          }
        });
      } catch (e) {
    ...
    (async () => {
      try {
        unlisten = await listen<{ count: number }>('share-received', (event) => {
          ...
          if (count > 0) {
            const label = count === 1 ? '1 个文件' : `${count} 个文件`;
            toast.success(`已收到 ${label}！可以上传了。`, { duration: 4000 });
          }
        });
      } catch (e) {
  ...
  const handleCheckLatency = useCallback(async () => {
    ...
    try {
      ...
      if (ms >= 0) {
        ...
        toast.success(`${emoji} 延迟：${ms}ms（到 Telegram 数据中心）`);
      } else {
        toast.error('无法连接到 Telegram 服务器');
      }
    } catch (e) {
  ...
  const handleUploadCachedFile = useCallback(async (entry: CachedFileEntry) => {
    ...
    try {
      ...
      toast.success(`已上传：${entry.file_name}`);
  ...
  const activeFolder = activeFolderId === null
    ? '已保存的消息'
    : folders.find(f => f.id === activeFolderId)?.name || '未知频道';
  ...
  const handleFolderVisibilityToggle = useCallback(async (folder: TelegramFolder) => {
    ...
    if (isPublic) {
      ...
      const username = prompt(`将“${folder.name}”设为公开。请输入用户名（留空则自动生成）：`, defaultUsername)?.trim();
  ...
  const handleFolderShareInvite = useCallback(async (folder: TelegramFolder) => {
    try {
      ...
      try {
        ...
        toast.success(`邀请链接已复制：${info.link}`);
      } catch (e) {
    ...
    <div className="absolute inset-0 flex flex-col bg-telegram-bg text-telegram-text overflow-hidden select-none font-sans">
      ...
      <header className="flex items-center justify-between px-5 pb-4 pt-[calc(1rem+env(safe-area-inset-top,24px))] bg-gradient-to-r from-telegram-hover/40 to-telegram-bg border-b border-telegram-border/60 shadow-lg backdrop-blur-md sticky top-0 z-40">
        <div className="flex items-center gap-3">
          <img src="/logo.svg" className="w-8 h-8 drop-shadow-lg" alt="标志" />
          <div>
        ...
        {activeTab === 'settings' && (
          <div className="space-y-4">
            <div className="p-4 rounded-2xl bg-telegram-hover/20 border border-telegram-border/30 space-y-4">
              <h3 className="text-sm font-bold text-telegram-primary tracking-wide uppercase text-[10px]">偏好设置</h3>
              <div className="flex items-center justify-between py-2">
                <div>
                  <p className="text-xs font-medium">上传前压缩文件夹</p>
                  <p className="text-[10px] text-telegram-subtext">上传前将文件夹压缩为 .zip</p>
                </div>
                <button
            ...
            <div className="p-4 rounded-2xl bg-telegram-hover/20 border border-telegram-border/30 space-y-4">
              ...
              <div className="flex items-center justify-between py-2 border-b border-telegram-border/20">
                <div>
                  <p className="text-xs font-medium">到 Telegram 的延迟</p>
              ...
              {latencyMs !== null && latencyMs >= 0 && (
                <div className="flex items-center gap-2 py-1">
                  ...
                  <span className={`text-[10px] font-semibold ${latencyMs < 100 ? 'text-green-400' : latencyMs < 250 ? 'text-yellow-400' : 'text-red-400'}`}>
                    {latencyMs < 100 ? '极佳' : latencyMs < 250 ? '良好' : '较慢'}
                  </span>
                </div>
              )}
              ...
              {bandwidth && (
                <div className="flex items-center justify-between py-2">
                  <div>
                    <p className="text-xs font-medium">数据用量（本次会话）</p>
                    <p className="text-[10px] text-telegram-subtext">连接以来的上传 / 下载</p>
                  </div>
            ...
            <div className="p-4 rounded-2xl bg-telegram-hover/20 border border-telegram-border/30 space-y-4">
              ...
              <div className="flex items-center justify-between py-2 border-b border-telegram-border/20">
                <div>
                  <p className="text-xs font-medium">启用代理</p>
                  <p className="text-[10px] text-telegram-subtext">通过代理服务器转发流量</p>
                </div>
                <button
              ...
              <div className="flex items-center justify-between py-2 border-b border-telegram-border/20">
                <div>
                  <p className="text-xs font-medium">代理类型</p>
                  <p className="text-[10px] text-telegram-subtext">SOCKS5（不支持 MTProto）</p>
                </div>
              ...
              <div className="flex items-center justify-between py-2 border-b border-telegram-border/20">
                <div>
                  <p className="text-xs font-medium">主机</p>
                  <p className="text-[10px] text-telegram-subtext">代理服务器地址</p>
                </div>
                <input
                  type="text"
              ...
              <div className="flex items-center justify-between py-2 border-b border-telegram-border/20">
                <div>
                  <p className="text-xs font-medium">端口</p>
                ...
                <>
                  <div className="flex items-center justify-between py-2 border-b border-telegram-border/20">
                    <div>
                      ...
                      <p className="text-[10px] text-telegram-subtext">可选</p>
                    </div>
                    <input
                      type="text"
                      placeholder="可选"
                  ...
                  <div className="flex items-center justify-between py-2">
                    <div>
                      <p className="text-xs font-medium">密码</p>
                      <p className="text-[10px] text-telegram-subtext">可选</p>
                    </div>
                    <input
                      type="password"
            ...
            <div className="p-4 rounded-2xl bg-telegram-hover/20 border border-telegram-border/30 space-y-4">
              <h3 className="text-sm font-bold text-telegram-primary tracking-wide uppercase text-[10px]">关于</h3>
              <div className="flex flex-col items-center py-3 space-y-4">
                <img src="/logo.svg" className="w-14 h-14 drop-shadow-lg" alt="Telegram Drive 标志" />
          ...
          >
            ...
            {bulkShareLoading ? (
              <div className="flex flex-col items-center justify-center py-12 space-y-3">
                ...
                <p className="text-xs text-telegram-subtext">正在生成分享链接...</p>
              </div>
            ) : (
