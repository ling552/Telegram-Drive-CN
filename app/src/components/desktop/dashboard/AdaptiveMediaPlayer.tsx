        ...
        if (video.canPlayType('application/vnd.apple.mpegurl')) {
            ...
            hls.on(Hls.Events.ERROR, (_event, data) => {
                if (data.fatal) {
                    ...
                    setHlsError(`HLS 播放错误：${data.details}`);
                    setHlsPhase('failed');
                    hls.destroy();
                    hlsRef.current = null;
                } else {
    ...
    return (
        ...
        >
            <div ref={containerRef} className={`relative ${isFullscreen ? 'fixed inset-0 w-screen h-screen max-w-none' : 'w-full max-w-6xl flex flex-col items-center'}`} onClick={e => e.stopPropagation()}>
                {/* Nav buttons */}
                <button onClick={onPrev} className={`absolute left-2 top-1/2 -translate-y-1/2 p-2 text-white/50 hover:text-white bg-white/10 hover:bg-white/20 rounded-full transition-all z-10 ${isFullscreen ? 'left-4' : ''}`} title="上一个（← / J）">
                    <ChevronLeft className="w-6 h-6" />
                </button>
                <button onClick={onNext} className={`absolute right-2 top-1/2 -translate-y-1/2 p-2 text-white/50 hover:text-white bg-white/10 hover:bg-white/20 rounded-full transition-all z-10 ${isFullscreen ? 'right-4' : ''}`} title="下一个（→ / L）">
                ...
                <div className={`bg-black overflow-hidden flex items-center justify-center relative ${isFullscreen ? 'w-full h-full rounded-none shadow-none ring-0' : 'w-full aspect-video rounded-xl shadow-2xl ring-1 ring-white/10'}`}>
                    ...
                    {(displayPhase === 'error' || displayPhase === 'failed') && (
                        <div className="flex flex-col items-center gap-3 text-white px-8">
                            ...
                            <p className="text-sm text-red-400 font-medium">播放错误</p>
                            <p className="text-xs text-white/40 text-center max-w-md">{displayError || '未知错误'}</p>
                            {isHlsMode && (
                        ...
                        <div className="flex flex-col items-center gap-4 text-white absolute inset-0 bg-black/80 z-10">
                            ...
                            <div className="flex flex-col items-center gap-1">
                                <p className="text-sm font-medium">
                                    {hlsPhase === 'preparing' ? `正在准备 ${hlsQuality}...` :
                                     hlsPhase === 'caching' ? '正在下载源文件...' :
                                     hlsPhase === 'transcoding' ? `正在转码为 ${hlsQuality}...` : ''}
                                </p>
                                {hlsProgress > 0 && (
                                ...
                                {hlsPhase === 'preparing' && <p className="text-[11px] text-white/30 mt-1">正在启动转码任务...</p>}
                            </div>
                            <button
                    ...
                    {showOriginalVideo && isMseLoading && (
                        <div className="flex flex-col items-center gap-4 text-white absolute inset-0 bg-black/80 z-10">
                            <Loader2 className="w-10 h-10 text-telegram-primary animate-spin" />
                            <div className="flex flex-col items-center gap-1">
                                <p className="text-sm font-medium">加载视频中</p>
                                {loadProgress > 0 && (
                                    <div className="flex items-center gap-2">
                                        <div className="w-32 h-1.5 bg-white/10 rounded-full overflow-hidden">
                                ...
                                <p className="text-[11px] text-white/30 mt-1">
                                    {msePhase === 'initializing' ? '正在初始化解码器...' : '正在解析视频元数据...'}
                                </p>
                            </div>
                        </div>
                    )}
                ...
                {(displayPhase === 'playing' || displayPhase === 'ready') && (
                    <div className={`absolute ${isFullscreen ? 'bottom-16 left-4' : 'top-3 right-3'} z-20 flex items-center gap-2`}>
                        ...
                        <div className="px-2.5 py-1 rounded-full bg-black/60 backdrop-blur-sm border border-white/10 text-xs font-medium text-white/90 shadow-lg pointer-events-none">
                            {isHlsMode && hlsQuality ? QUALITY_LABELS[hlsQuality] : `${effectiveQuality === 'original' ? '原画' : QUALITY_LABELS[effectiveQuality]}${measuredKbps > 0 && effectiveQuality !== 'original' ? ` · ${(measuredKbps / 1000).toFixed(0)}k` : ''}`}
                        </div>
                        {/* Resolution badge */}
                ...
                {debugOverlay && (
                    <div className={`absolute z-40 pointer-events-none ${isFullscreen ? 'bottom-20 left-4' : 'bottom-4 left-4'}`}>
                        <div className="px-3 py-2 rounded-lg bg-black/80 backdrop-blur-sm border border-white/10 text-[10px] font-mono text-white/80 shadow-xl space-y-1">
                            ...
                            <div className="flex justify-between gap-4">
                                <span className="text-white/40">上限</span>
                                <span>{effectiveQuality === 'original' ? '无限制' : `${QUALITY_LABELS[effectiveQuality]}${!transcodeCapabilities?.available ? '（限速）' : ''}`}</span>
                            </div>
                            <div className="flex justify-between gap-4">
                                <span className="text-white/40">已缓冲</span>
                            ...
                            <div className="flex justify-between gap-4">
                                ...
                                <span className={isHlsMode ? 'text-emerald-400' : effectiveQuality !== 'original' && !transcodeCapabilities?.available ? 'text-amber-400' : 'text-white/60'}>
                                    {isHlsMode ? 'HLS' : effectiveQuality !== 'original' && !transcodeCapabilities?.available ? '带宽限制' : '原画'}
                                </span>
                            </div>
                            {(sourceResolution || playingResolution) && (
                                <div className="flex justify-between gap-4">
                                    ...
                                    <span>
                                        ...
                                        {playingResolution && sourceResolution && (playingResolution.w !== sourceResolution.w || playingResolution.h !== sourceResolution.h)
                                            ? ` (源：${sourceResolution.w}×${sourceResolution.h})`
                            ...
                            <div className="pt-1.5 mt-1 border-t border-white/5">
                                <button
                                    ...
                                    title={`清除 ${fileKey} 的所有转码 HLS 版本`}
                                >
                                    <Trash2 className="w-3 h-3" />
                                    {clearingCache ? '清除中...' : '清除转码缓存'}
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="absolute bottom-0 left-0 right-0 z-30 bg-gradient-to-t from-black/90 via-black/60 to-transparent p-4 pt-12 pointer-events-none">
                        <div className="flex items-center justify-between pointer-events-auto">
                            <div className="flex items-center gap-3">
                                ...
                                >
                                    <button onClick={toggleMute} className="p-1.5 text-white/60 hover:text-white rounded-full hover:bg-white/10" title={isMuted ? '取消静音' : '静音'}>
                ...
                {!isFullscreen && <div className="mt-3 w-full flex items-center justify-between px-1 gap-3">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded border ${
                            ...
                            {isHlsMode ? 'HLS' : effectiveQuality !== 'original' && !transcodeCapabilities?.available ? '已限速' : '原画'}
                        </span>
                ...
                {!isFullscreen && <div className="mt-2 flex items-center gap-4 text-[10px] text-white/25 select-none">
                    <span className="flex items-center gap-1">
                        <kbd className="px-1 py-0.5 rounded bg-white/10 text-white/40 text-[9px] font-mono">← →</kbd> 切换
                    </span>
                    <span className="flex items-center gap-1">
                        <kbd className="px-1 py-0.5 rounded bg-white/10 text-white/40 text-[9px] font-mono">Space</kbd> 播放/暂停
                    </span>
                    <span className="flex items-center gap-1">
                        <kbd className="px-1 py-0.5 rounded bg-white/10 text-white/40 text-[9px] font-mono">F</kbd> 全屏
                    </span>
                    <span className="flex items-center gap-1">
                        <kbd className="px-1 py-0.5 rounded bg-white/10 text-white/40 text-[9px] font-mono">Esc</kbd> 关闭
                    </span>
                    <span className="flex items-center gap-1">
                        <kbd className="px-1 py-0.5 rounded bg-white/10 text-white/40 text-[9px] font-mono">M</kbd> 静音
                    </span>
                    <span className="flex items-center gap-1">
                        <kbd className="px-1 py-0.5 rounded bg-white/10 text-white/40 text-[9px] font-mono">D</kbd> 调试
                    </span>
                </div>}
            </div>
        </div>
