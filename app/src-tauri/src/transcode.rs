...
pub async fn execute_transcode_pipeline(
    ...
    let ffmpeg_path = match ffmpeg_path {
        ...
        None => {
            ...
            job.phase = JobPhase::Error("FFmpeg 不可用".to_string());
            return;
        }
    };
    ...
    if !original_path.exists() {
        ...
        match cache_original(
            ...
            Err(e) => {
                let mut job = job_arc.lock().await;
                job.phase = JobPhase::Error(format!("缓存失败：{}", e));
                return;
            }
        }
    }
...
pub async fn cmd_prepare_transcoded_stream(
    ...
    let preset = QUALITY_PRESETS
        ...
        .ok_or_else(|| format!("未知的清晰度：{}", quality))?;
    ...
    let client = client.ok_or_else(|| "未连接到 Telegram".to_string())?;
    ...
    let media = msg.media().ok_or_else(|| "无媒体内容".to_string())?;
...
pub async fn cmd_clear_transcode_cache(
    ...
    match (file_key, quality) {
        ...
        (None, None) => {
            ...
            Ok(format!("已清除全部转码缓存（{} 项）", removed_count))
        }
        ...
        (Some(fk), Some(q)) => {
            ...
            Ok(format!("已清除 {} 的 {} 清晰度变体", fk, q))
        }
        (None, Some(_)) => Err("未指定 file_key 时无法清除清晰度".to_string()),
    }
}
