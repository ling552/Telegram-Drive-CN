...
<head>
    <meta charset="utf-8">
    <title>密码保护文件 - Telegram Drive</title>
    <style>
        body {{
...
<body>
    <div class="container">
        <h2>请输入密码</h2>
        <p>此分享链接受密码保护。<br>文件：<strong>{}</strong></p>
        {}
...
async fn get_shared_file(
    ...
    let row = match get_share_by_token(&db_conn, &token) {
        Ok(Some(r)) => r,
        Ok(None) => return HttpResponse::NotFound().body("未找到分享链接"),
        Err(e) => {
            log::error!("DB error resolving token {}: {}", token, e);
            return HttpResponse::InternalServerError().body("服务器内部错误")
        }
    };

    // Check validation (revocation and expiration)
    if row.revoked {
        return HttpResponse::NotFound().body("此分享链接已被撤销");
    }

    if let Some(expiry) = row.expires_at {
        let now = chrono::Utc::now().timestamp();
        if expiry < now {
            return HttpResponse::Gone().body("此分享链接已过期");
        }
    }
    ...
    let client = match client_opt {
        Some(c) => c,
        None => return HttpResponse::ServiceUnavailable().body("Telegram 客户端未连接"),
    };
    ...
    let peer = match resolve_peer(&client, row.folder_id, &tg_state.peer_cache).await {
        ...
        Err(e) => {
            ...
            return HttpResponse::InternalServerError().body("无法定位文件夹");
        }
    };
    ...
    match client.get_messages_by_id(peer, &[row.message_id]).await {
        Ok(messages) => {
            ...
            HttpResponse::NotFound().body("在 Telegram 中未找到消息或媒体")
        }
        Err(e) => {
            log::error!("Failed to fetch shared message {}: {}", row.message_id, e);
            HttpResponse::InternalServerError().body(format!("获取文件失败：{}", e))
        }
    }
}
...
async fn verify_shared_file_password(
    ...
    let hash = match &row.password_hash {
        Some(h) => h,
        None => return HttpResponse::BadRequest().body("此链接无需密码"),
    };
    ...
    if verify_password(&form.password, hash) {
        ...
        render_password_form(&row.file_name, &token, Some("密码错误，请重试。"))
    }
}
